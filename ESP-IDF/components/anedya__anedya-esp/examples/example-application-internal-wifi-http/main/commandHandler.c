...
static void
update_command_status(anedya_req_cmd_status_update_t *command_status_update);

void commandHandling_task(void *pvParameters) {
  current_task = xTaskGetCurrentTaskHandle();
  // Wait for the connection to be established before sending requests
  xEventGroupWaitBits(ConnectionEvents, ANEDYA_CONNECTED_BIT, pdFALSE, pdFALSE,
                      portMAX_DELAY);
  ...
  anedya_err_t err =
      anedya_op_cmd_get_list(&anedya_client, &cmd_list_txn, cmd_list_req);
  if (err != ANEDYA_OK) {
      ...
      if (cmd_list_resp.is_available) {
        ...
        for (int i = 0; i < cmd_list_req.limit; i++) {
          ESP_LOGI(TAG,
                   ...
                   "02X%02X",
                   cmd_list_resp.commands[i].cmdId[0],
                   cmd_list_resp.commands[i].cmdId[1],
                   cmd_list_resp.commands[i].cmdId[2],
                   cmd_list_resp.commands[i].cmdId[3],
                   cmd_list_resp.commands[i].cmdId[4],
                   cmd_list_resp.commands[i].cmdId[5],
                   cmd_list_resp.commands[i].cmdId[6],
                   cmd_list_resp.commands[i].cmdId[7],
                   cmd_list_resp.commands[i].cmdId[8],
                   cmd_list_resp.commands[i].cmdId[9],
                   cmd_list_resp.commands[i].cmdId[10],
                   cmd_list_resp.commands[i].cmdId[11],
                   cmd_list_resp.commands[i].cmdId[12],
                   cmd_list_resp.commands[i].cmdId[13],
                   cmd_list_resp.commands[i].cmdId[14],
                   cmd_list_resp.commands[i].cmdId[15]);
  ...
  if (ulNotifiedValue == 0x01) {
    if (cmd_next_txn.is_success && cmd_next_txn.is_complete) {
      if (cmd_next_resp
              .is_available) { // If the command is available, log the details

        ESP_LOGI(TAG, "Available: %s",
                 cmd_next_resp.is_available ? "true" : "false");
        ...
        ESP_LOGI(TAG, "Next Available: %s",
                 cmd_next_resp.nextavailable ? "true" : "false");
      } else {
  ...
  while (1) {
    ...
    xEventGroupWaitBits(OtaEvents, OTA_NOT_IN_PROGRESS_BIT, pdFALSE, pdFALSE,
                        portMAX_DELAY);
    xEventGroupWaitBits(gatewaystate.COMMANDEVENTS, COMMAND_AVAILABLE_BIT,
                        pdFALSE, pdFALSE, portMAX_DELAY);

    // Update the command status object
    memset(&command_status_update, 0,
           sizeof(command_status_update)); // Clear the structure
    memcpy(command_status_update.cmdId, command_obj->cmdId,
           sizeof(command_status_update.cmdId));
...
update_command_status(anedya_req_cmd_status_update_t *command_status_update) {
  ...
  ESP_LOGI(TAG, "Status update: %s",
           command_status_update->status); // Updated for string status
