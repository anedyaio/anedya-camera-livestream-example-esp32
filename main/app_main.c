...
#include "boards.h"                     // camera pin map (board selected in menuconfig)
...
void app_main(void)
{
    ...
    // libpeer's SCTP/data-channel plumbing logs every TSN and heartbeat at INFO,
    // which floods the console (thousands of "BUF_MNGR: Set forward tsn" /
    // "SCTP: Receive chunk" lines) and buries the events we care about. Mute them
    // to WARN. AGENT stays at INFO so TURN permission/channel-bind refreshes and
    // any relay failures remain visible.
